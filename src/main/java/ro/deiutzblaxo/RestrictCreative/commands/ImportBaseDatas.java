        ...
        if (sender instanceof ConsoleCommandSender) {
            if (args.length == 2) {
                if (args[0].equalsIgnoreCase("local") && args[1].equalsIgnoreCase("online")) {
                    if (MySQLManager.basedataOnline) {
                    ...
                    if (MySQLManager.basedataOnline) {
                        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
                            ...
                            public void run() {
                                ...
                                Connection con;
                                try {
                                    String sql = "CREATE TABLE " + MySQLManager.table + "(LOCATION TEXT(1000))";
                                    ...
                                    for (String loc : plugin.getDataService().getLocations()) {

                                        PreparedStatement statement;

                                        statement = con
                                                .prepareStatement("SELECT * FROM " + MySQLManager.table
                                                        + " WHERE LOCATION =(?)");
                                        statement.setString(1, loc);
                                        ResultSet results = statement.executeQuery();
                                        if (!results.next()) {
                                            PreparedStatement insert = con.prepareStatement(
                                                    "INSERT INTO " + MySQLManager.table + " (LOCATION) VALUES (?)");
